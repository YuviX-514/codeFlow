    ...
    <section className="relative py-24 overflow-hidden">
      ...
      <div className="absolute inset-0 -z-10 overflow-hidden">
        ...
        <motion.div
          ...
          className="absolute inset-0 bg-[radial-gradient(circle_at_center,_rgba(168,85,247,0.3)_0%,_transparent_70%)]"
        />
      </div>
      ...
      <div className="container px-4 mx-auto max-w-6xl">
        ...
        <div className="relative">
          {/* Animated timeline with gradient pulse */}
          <motion.div
            initial={{ height: 0 }}
          ...
          >
            ...
            <motion.div
              ...
              className="absolute top-0 left-0 w-full h-32 bg-gradient-to-b from-transparent via-white/30 to-transparent"
            />
          </motion.div>
          ...
          >
            <motion.div
              ...
              className="absolute top-0 left-0 w-px h-full bg-purple-500/30 shadow-[0_0_20px_8px_rgba(168,85,247,0.3)]"
            />
          </motion.div>
          ...
          {workflowSteps.map((step, index) => (
            ...
            >
              {/* Animated timeline dot with color */}
              <motion.div
                initial={{ scale: 0 }}
                whileInView={{ scale: 1 }}
                transition={{ delay: index * 0.1 + 0.3, type: "spring", stiffness: 500 }}
                className={`absolute left-6 top-5 md:left-1/2 md:-translate-x-1/2 w-3 h-3 rounded-full ${step.color} shadow-lg z-10`}
              >
                <motion.div
                  ...
                  <div className="relative z-10">
                    <div className="flex items-center gap-3 mb-3">
                      <motion.span
                        ...
                        className={`text-xs font-medium py-1 px-2 rounded-full ${step.color}/20 text-${step.color.split('-')[1]}-300`}
                      >
                        Step {index + 1}
                      </motion.span>
                      <motion.span 
                    ...
                    <motion.h3
                      ...
                      whileInView={{ opacity: 1, y: 0 }}
                      transition={{ delay: index * 0.1 + 0.4 }}
                      className={`text-xl font-semibold text-white mb-2 group-hover:text-${step.color.split('-')[1]}-300 transition-colors`}
                    >
                      {step.title}
                    </motion.h3>
                    <motion.p
